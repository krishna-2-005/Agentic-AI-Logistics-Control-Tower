...
| Curated demo samples | 5 pairs in `demo/sample_documents/` — 1 clean + 1 per error kind |
| Tests | 16 in `tests/test_doc_corpus.py`, all passing; `ruff` clean |

## 6. What's next

- **Lahari's sign-off on D-020** — the taxonomy and the GSTIN-validity call are
  Krishna's proposal, not yet a confirmed team decision.
